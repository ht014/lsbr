...
detector = RelModel(classes=train.ind_to_classes, rel_classes=train.ind_to_predicates,
                    ...
                    use_bias=conf.use_bias,reachability=conf.reachability,
...
if 'disc_center.centroids' in ckpt['state_dict'].keys():
    detector.centroids = ckpt['state_dict']['disc_center.centroids'].data
if conf.ckpt.split('-')[-2].split('/')[-1] == 'vgrel':
    print("Loading EVERYTHING")
    start_epoch = -1#ckpt['epoch']
...
def train_epoch(epoch_num):
    ...
    for b, batch in enumerate(train_loader):
        res0, multi_loss,multi_rel_loss,result,portion = train_batch(batch, multi_loss,multi_rel_loss,verbose=b % (conf.print_interval*10) == 0,portion=portion)
        tr.append(res0)
        ...
        if b % conf.print_interval == 0 and b >= conf.print_interval:
            ...
            if multi_loss is not None:
                 print('multi-label loss: ', multi_loss.data.cpu().numpy()/portion)
            else:
                print('multi-label loss: ', 'None')

            # print("Augment ratio: ", num_aug*1.0/sum_rels)
            print('-----------', flush=True)
            start = time.time()
    return pd.concat(tr, axis=1)

def convert(labels, KK=151):
    a = labels.data.cpu().numpy()
    p = np.zeros((1,KK))
    for i in a:
       p[0][i] = 1
    return torch.from_numpy(p).float().cuda().squeeze()
...
def train_batch(b, multi_l ,multi_rel,verbose=False, portion=0):
    ...
    losses['rel_loss'] = F.cross_entropy(result.rel_dists, result.rel_labels[:, -1])  # ,to_cuda(W_REL))
...
def val_batch(batch_num, b, evaluator):
    ...
    for i, (boxes_i, objs_i, obj_scores_i, rels_i, pred_scores_i) in enumerate(det_res):
        # print(val.relationships[batch_num + i].copy().shape)
        gt_entry = {
            'gt_classes': val.gt_classes[batch_num + i].copy(),
            'gt_relations': val.relationships[batch_num + i].copy(),
            'gt_boxes': val.gt_boxes[batch_num + i].copy(),
        }
        #if gt_entry['gt_relations'].shape[0] ==0: continue
        # print(gt_entry['gt_relations'].shape)
