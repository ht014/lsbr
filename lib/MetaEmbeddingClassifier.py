...
from lib.CosNormClassifier import CosNorm_Classifier
...
class MetaEmbedding_Classifier(nn.Module):
    ...
    def forward(self, x, centroids, *args):
        ...
        max_prob = torch.max(values_memory[:,1:],dim=-1)
        # print(values_memory.shape,max_prob[0].shape,max_prob[1].shape)
        reachability = scale * max_prob[0].unsqueeze(1).expand(-1, feat_size)

        # print((scale / values_nn[:, 0]).unsqueeze(1).shape,reachability.shape,values_memory.shape,'shape of reach')
        # input()
        ...
        return logits, [direct_feature, infused_feature,values_memory]
